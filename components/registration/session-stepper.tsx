...
import {
  saveServiceSessionCorrectionAction,
  saveSessionCorrectionAction,
...
export function ServiceSessionStepper({
  compact,
  ...
  compact: boolean;
  customerPackageServiceId: string;
  ...
  return (
    <form
      action={saveServiceSessionCorrectionAction}
      className="mt-4 border-t border-border pt-3"
    >
      <input
        name="customerPackageServiceId"
        type="hidden"
        value={customerPackageServiceId}
      />
      <input
        name="previousRemainingSessions"
        type="hidden"
      ...
      {compact ? <input name="view" type="hidden" value="compact" /> : null}
      <p className="text-xs font-bold uppercase tracking-wide text-secondary">
        Manual service correction
      </p>
      <div className="mt-2 grid grid-cols-[2.75rem_minmax(0,1fr)_2.75rem] items-center gap-2">
        <button
          aria-label={`Decrease ${serviceName} remaining sessions`}
          className="flex size-11 items-center justify-center rounded-lg bg-neutral text-xl font-bold text-foreground transition-colors hover:bg-neutral-hover disabled:cursor-not-allowed disabled:opacity-50"
          disabled={draftSessions === 0}
          onClick={() =>
            setDraftSessions((currentSessions) =>
        ...
        <input
          aria-label={`${serviceName} new remaining sessions`}
          className="min-h-11 w-full min-w-0 rounded-lg border border-input-border bg-page px-2 py-2 text-center font-bold text-foreground outline-none focus:border-brand focus:ring-2 focus:ring-soft-blue"
          max={initialSessions}
          min={0}
          name="newRemainingSessions"
          onChange={(event) => {
        ...
        />
        <button
          aria-label={`Increase ${serviceName} remaining sessions`}
          className="flex size-11 items-center justify-center rounded-lg bg-neutral text-xl font-bold text-foreground transition-colors hover:bg-neutral-hover disabled:cursor-not-allowed disabled:opacity-50"
          disabled={draftSessions >= initialSessions}
          onClick={() =>
            setDraftSessions((currentSessions) =>
              Math.min(initialSessions, currentSessions + 1),
            )
          }
          type="button"
        >
      ...
      >
        {hasUnsavedChanges
          ? `Unsaved draft: ${draftSessions} sessions.`
          : "Saved service count. Changes remain draft until saved."}
      </p>
      <Button
        className="mt-3 w-full"
        disabled={!hasUnsavedChanges || draftSessions > initialSessions}
        pendingLabel="Saving..."
        type="submit"
      >
        Save service correction
      </Button>
    </form>
  );
}
